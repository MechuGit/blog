...
import (
	...
	"github.com/mechu/blog/testutil/network"
	"github.com/mechu/blog/x/nameservice/client/cli"
...
func networkWithWhoisObjects(t *testing.T, n int) (*network.Network, []types.Whois) {
	...
	for i := 0; i < n; i++ {
		state.WhoisList = append(state.WhoisList, types.Whois{
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Whois)
			}
		})
	}
}
...
func TestListWhois(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			require.Subset(t, objs, resp.Whois)
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Whois)
	})
}
